...
const ACTION_LABELS = {
  ...
  analyze: { title: "Analyze with AI",     color: "violet", icon: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
...
const CONFIRM_STYLES = {
  blue:   "bg-blue-600 hover:bg-blue-500 disabled:bg-blue-900 disabled:text-blue-400",
  violet: "bg-violet-600 hover:bg-violet-500 disabled:bg-violet-900 disabled:text-violet-400",
};
    ...
    >
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-slate-800 flex items-center gap-3">
          <div className={`p-2 rounded-lg ${action === "analyze" ? "bg-violet-500/10 text-violet-400" : "bg-blue-500/10 text-blue-400"}`}>
            {meta.icon}
          </div>
          <div>
            <h2 className="text-sm font-semibold text-slate-100">{meta.title}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              className={`w-full bg-slate-800 border rounded-lg px-3 py-2.5 text-sm text-slate-200
          ...
          <div className="flex gap-3 pt-1">
            <button
              ...
              onClick={onCancel}
              disabled={loading}
              className="flex-1 py-2.5 rounded-lg border border-slate-700 text-sm text-slate-400
                hover:bg-slate-800 hover:text-slate-200 disabled:opacity-50 cursor-pointer
